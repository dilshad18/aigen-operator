...
MIN_RECONCILE_INTERVAL = 60  # Minimum 60 seconds between node-triggered reconciles
...
def on_node_event(type, name, **_):
    """
    React to node events with debouncing to prevent excessive reconciliations.
    - ADDED/DELETED: Always trigger immediate reconciliation
    - MODIFIED: Only trigger if MIN_RECONCILE_INTERVAL has passed since last reconcile
    """
    ...
    if type in ['ADDED', 'DELETED']:
        ...
        LOG.info(f"Node {type}: {name} - triggering immediate reconciliation")
        reconcile()
        remove_node_finalizers()
        last_reconcile_time = current_time
    elif type == 'MODIFIED':
        if time_since_last >= MIN_RECONCILE_INTERVAL:
            # Enough time has passed, reconcile
            LOG.info(f"Node MODIFIED: {name} - triggering reconciliation ({time_since_last:.1f}s since last)")
            reconcile()
            remove_node_finalizers()
            last_reconcile_time = current_time
        else:
            # Too soon, skip this event
            LOG.debug(f"Node MODIFIED: {name} - skipping reconciliation (last reconcile {time_since_last:.1f}s ago, min interval: {MIN_RECONCILE_INTERVAL}s)")
    else:
